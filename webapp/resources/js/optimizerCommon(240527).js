...
function selectResourceListByParentId(resource_parent_no){
	...
	table_resource = new Tabulator("#volist", {
	    ...
	    ajaxResponse:function(url,prarm,response){
	    	...
	    	for(let i=0;i<response.data.length;i++){
	    		...
	    		response.data[i].optimize_btn = `<button class="btn btn-sm btn-single-optimize">최적화</button>`;
	    		//let rand = Math.random() < 0.5 ? 0 : 1;
	    		if(response.data[i].resource_condition == 1){
	    			response.data[i].condition = `<span class="badge badge-normal"><ion-icon name="checkmark-circle"></ion-icon> 양호</span>`;	    			
	    		}else{
	    			response.data[i].condition = `<span class="badge badge-abnormal"><ion-icon name="close-circle"></ion-icon> 불량</span>`;
	    		}
	    	}
	    	return response; 
	    },
	    ...
	    columns: [
	    	...
	    	},
	    	{
	    		...
	    		formatter: function(cell, formatterParams, onRendered) {
	    			...
	    			const foundItem = statusArray.find(item => item.value === cell.getValue());
	    			switch(cell.getValue()){
	    				case 1 : 
	...
	table_resource.on("dataLoaded", function() {
		...
		if ($(".optimize-button-container").length === 0) {
		    // .optimize-button-container 요소가 없는 경우에만 버튼을 추가
		    let buttons = `<div class="optimize-button-container">
		        <button class="btn btn-sm"><ion-icon name="checkmark-done-circle"></ion-icon> 전체 항목 최적화</button>
		        <button class="btn btn-sm btn-secondary"><ion-icon name="close-circle"></ion-icon> 선택 항목 최적화 해제</button>
		    </div>`;
		    $(".tabulator-footer-contents").append(buttons);
		}
		...
		setTimeout(function(){
			...
			const resource_name = urlParams.get("resource_name");
			const resource_no = urlParams.get("resource_no");		
			// const targetRow = table_resource.getData().find(row => row.resource_no == resource_no);
			let targetRow = null;
			let targetEle = null;
	...
	table_resource.on("cellClick", function(e, cell) {
	    if (cell.getField() === "detail_btn" || cell.getField() === "resource_name") {// 특정 컬럼일 때만 함수 호출
	    	resourceDetailEvent(cell.getRow());
	    }
	});			
}
...
//단위 설정
function formatTrafficData(bytes) {
    ...
    return `${bytes.toFixed(1).toLocaleString()} ${units[unitIndex]}`;
}


function resourceDetailEvent(row) {
	
	// 데이터 가져오기
    let resource_no = row.getData().resource_no;	
    let data = selectResourceByResourceNo(resource_no);
    
    let resource_status = data.resource_status;
    let $rowElement = $(row.getElement());
    let $detailElement = $rowElement.next(".resource-detail");

    /*절약 트래픽 구하기*/
    var saved_traffic = savedTraffic();
    var selectedResourceName = row.getData().resource_name;
    var selectedItem = saved_traffic.data.find(item => item.resource_name === selectedResourceName);
    var optimizedbeforetraffic = selectedItem.optimizedbeforetraffic;
    var optimizedaftertraffic = selectedItem.optimizedaftertraffic;
    
	 /*최적화 상태 구하기*/
    let trafficData = optimizedbeforetraffic - optimizedaftertraffic;
    trafficData = formatTrafficData(trafficData);
    trafficData = `<img src="/resources/img/icon-down.png" />${trafficData}`;
    if(resource_status != 1){
    	trafficData = "-";
    }
    
    /*로딩 속도 향상률 구하기*/
    var speedIncreaseRate = ((1-(optimizedaftertraffic/optimizedbeforetraffic))*100).toFixed(1);
    if(isNaN(speedIncreaseRate)){
    	speedIncreaseRate = 0;
	}
    speedIncreaseRate = `<img src="/resources/img/loding.png" / style="width: 38px; height: 38px;">${speedIncreaseRate}%`;
    if(resource_status != 1){
    	speedIncreaseRate = "-";
    }
    ...
    //이미지
    let previewData = resourcePrintByType(data);
    //${previewData.col1}
    //${previewData.col2}
   ...
   /* {value : 1, label : "최적화 완료", icon : "checkmark-circle"},
    ...
    switch(resource_status){
		case -1 : 
			result_status = `<ion-icon name="remove-circle" role="img" class="md hydrated" style="color: gray; font-size:1.5rem;"></ion-icon><h2 style="color: gray; font-size:1.6rem;">최적화 미적용</h2>`;
			break;
		case 0 : 
			result_status = `<ion-icon name="pause-circle" role="img" class="md hydrated" style="color: #f1c232; font-size:1.5rem;"></ion-icon><h2 style="color: #f1c232; font-size:1.8rem;">최적화 대기</h2>`;
			break;
		case 1 : 
			result_status = `<ion-icon name="checkmark-circle" role="img" class="md hydrated" style="color: #1E90FF; font-size: 1.5rem;"></ion-icon><h1 style="color: #1E90FF; font-size: 1.8rem;">최적화 완료</h1>`	
			break;
		case 2 : 
			result_status = `<ion-icon name="remove-circle" role="img" class="md hydrated" style="color: #1E90FF; font-size: 1.5rem;"></ion-icon><h1 style="color: #1E90FF; font-size: 1.8rem;">최적화 해제</h1>`	
			break;
	    case 11 : 
			result_status = `<ion-icon name="play-circle" role="img" class="md hydrated" style="color: #51d28c; font-size: 1.5rem;"></ion-icon><h1 style="color: #51d28c; font-size: 1.5rem;">최적화 진행 중 </h1>`	
			break;
    }
    
    let html = `<div class='resource-detail' style='display:none; height:auto; background:rgba(255,255,255,0.05); border-radius:5px; padding:20px;'>
    				<div class="p-2">
    					<h4 class="mb-2">최적화 상세 정보</h4>
    				 	<p><i class="uil uil-exclamation-octagon mx-1" style="color: #bcbcbc;"></i>지난 24시간 기준</p></div>`;
    html += `<div class="card" style="background:transparent; margin:0px 0px 20px 0px;">
				<div class="card-body d-flex">
				<div class="col-4">
					<h2 class="card-title">압축 방식<i class="uil uil-info-circle me-2"style="color: #00CCFF;"></i></h2>
					<div class="card-content mt-3">
						<div id="algorithmStatus">
							<span class="mb-0">
								<h1 style="color: #FCC200; margin-right: 10px; font-size: 50px;">deflate</h1>
							</span> <span> 현재 압축률이 가장 우수한 <br> <strong style="color: #FCC200;">deflate 무손실 압축 알고리즘</strong>을
								적용하였습니다.
							</span>
						</div>
					</div>
				</div>
				<div class="col-2 list-group-item-2">
					<h3 class="card-title">최적화 적용 여부<i class="uil uil-info-circle me-2" style="color: #00CCFF;"></i>
					</h3>									
					<div class="card-content d-flex mt-5">
							${result_status}	
					</div>															
				</div>`;
    if(data.resource_condition == 1){
    	html += `
				<div class="savedTraffic col-3 list-group-item-2">
					<h3 class="card-title">
						절약 트래픽<i class="uil uil-info-circle me-2"
							style="color: #00CCFF;"></i>
					</h3>
					<div class="down-img mt-3 mb-2 d-flex justify-content-center">
						<h1 class="small_title">${trafficData}<span style="font-size: 30px;"></span></h1>
					</div>
					<p class="text-center mb-0">
						웹 콘텐츠 최적화를 통해 절약된 <br>트래픽 용량입니다.
					</p>
				</div>
				<div class="col-3 list-group-item-2">
					<h3 class="card-title">로딩 속도 향상률<i class="uil uil-info-circle me-2" style="color: #00CCFF;"></i></h3>
					<div class="loding-img mt-3 mb-2 d-flex justify-content-center">
						<h1 class="small_title">${speedIncreaseRate}</h1>
					</div>
					<p class="text-center mb-0">
						웹 콘텐츠 최적화를 통해 향상된 <br>로딩 속도 비율입니다.
					</p>
				</div>`;
    }else if(data.resource_condition == -1){
    	html += `<div class="col-6 list-group-item-2">
    		<h3 class="card-title">최적화 상태 <i class="uil uil-info-circle me-2" style="color: #00CCFF;"></i></h3>
    		<div class="d-flex">
    			<div class="col-6 narrow" style="padding:30px 0px 0px 0px;">
    				<h1 style="color:var(--color-red); font-size:40px;"><img src="/resources/img/icon-warning-red.png" width="35px" style="position:relative; top:-5px;" /> 파일 용량 과다로 비용 증가</h1>
    				<p>웹 콘텐츠 최적화를 진행했으나, 압축률이 낮아<br />로딩 속도 및 비용 절감의 효과가 미비 하거나 없음!</p>
    			</div>
    		</div>
    	</div>`;
    }
    ...
    html += `<h5 class="card-title mx-2">웹 콘텐츠 미리보기</h5>
						<div class="d-flex mt-3 mb-2">
							<div class="col-6">
								<div class="card" style="background:transparent;">
									<div class="card-header" style="background:transparent;">
										<h6 class="my-0 text-center">최적화 전</h6>
										<button class="btn-ori btn btn-sm btn-primary">원본 보기</button>
									</div>
							...
							<div class="col-6">
								<div class="card" style="background:transparent;">
									<div class="card-header" style="background:transparent;">
										<h6 class="my-0 text-center">최적화 후</h6>
										<button class="btn-ori btn btn-sm btn-primary">원본 보기</button>
									</div>
									${previewData.col2}
								</div>
						...
						<h5 class="card-title mb-3 mx-2">추천 알고리즘 선택</h5>
						<div class="card p-2" style="background:transparent;">
							<div class="row d-flex align-items-center">
								<div class="col-md-4 d-flex align-items-center" >
									<h6 class="p-3 text-center">
										이미지 파일<br> 압축 알고리즘
									</h6>
									<div class="nav list-group-item-2 mx-2 flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical" style="width: 245px;">
										<a class="nav-link mb-2" id="v-pills-home-tab" data-bs-toggle="pill" href="#Mozjpeg" role="tab" aria-controls="v-pills-home" aria-selected="false" tabindex="-1">Mozjpeg</a>
										<a class="nav-link mb-2" id="v-pills-profile-tab" data-bs-toggle="pill" href="#zlib" role="tab" aria-controls="v-pills-profile" aria-selected="false" tabindex="-1">zlib</a>
										<a class="nav-link mb-2" id="v-pills-messages-tab" data-bs-toggle="pill" href="#LZW" role="tab" aria-controls="v-pills-messages" aria-selected="false" tabindex="-1">LZW</a>
										<a class="nav-link active" id="v-pills-settings-tab" data-bs-toggle="pill" href="#deflate" role="tab" aria-controls="v-pills-settings" aria-selected="true" style="position:relative;">deflate <span class="badge-recommand"><img src="/resources/img/icon-sparkle.png" width="16px" style="position:relative; top:-1px;" /> 추천</span></a>
									</div>
								</div>
								<div class="col-md-8">
									<div class="tab-content text-muted mt-4 mt-md-0" id="v-pills-tabContent">
										<div class="tab-pane fade d-flex justify-content-center" 
										id="Mozjpeg" role="tabpanel" aria-labelledby="v-pills-home-tab"></div>
										<div class="tab-pane fade d-flex justify-content-center" 
										    id="zlib" role="tabpanel" aria-labelledby="v-pills-profile-tab"></div>
										<div class="tab-pane fade d-flex justify-content-center" 
										     id="LZW" role="tabpanel" aria-labelledby="v-pills-messages-tab"></div>
										     										
										<div class="tab-pane fade active show d-flex justify-content-center"
											id="deflate" role="tabpanel" aria-labelledby="v-pills-settings-tab">

											<div class="list-group-item-2 p-2">
												<h5 class="font-size-16">
													웹 콘텐츠 예상 절약 트래픽<i class="uil uil-info-circle me-2"
														style="color: #00CCFF;"></i>
												</h5>
												<div class="text-center mt-4">
												<div class="down-img mt-4 d-flex justify-content-center">
													<h1 class="mb-4">${trafficData}</h1>
												</div>
												<span>현재 알고리즘을 기준으로 <br>예상 절약 트래픽
														용량을 나타냅니다.
													</span>
											</div></div>
											...
											<div class="list-group-item-2 p-2">
												<h5 class="font-size-16">
													웹 콘텐츠 예상 로딩 속도 향상률<i class="uil uil-info-circle me-2"
														style="color: #00CCFF;"></i>
												</h5>
												<div class="text-center mt-4">
													<div class="loding-img d-flex justify-content-center mb-3">
														<h1>${speedIncreaseRate}</h1>
													</div>
													<span>현재 알고리즘을 기준으로 <br>예상 로딩 속도의 비율을 나타냅니다.
													</span>
												</div>
											</div>
											<div class="list-group-item-2 p-2">
												<h5 class="font-size-16">
													웹 콘텐츠 이미지 해상도<i class="uil uil-exclamation-triangle me-2"
														style="color: yellow;"></i>
												</h5>
												<div
													class="d-flex align-items-center">
													<span class="m-2"><span style="font-size: 1.2em;">🖼️</span>원본
														이미지 해상도</span>
													<h4>1920 x 1080</h4>
												</div>
												<div
													...
													style="color: yellow;">
													<span class="m-2"><span style="font-size: 1.2em;">🖼️</span>최적화
														후 이미지 해상도</span>
													<h4 style="color: yellow;">1600 x 900</h4>
												</div>
												<div class="text-center">
												<span>최적화 알고리즘에 따라 <br> 해상도가 <strong style="color: yellow;">저하</strong> 될 수 있습니다!</span></div>
											</div>
										</div>
									</div>
								</div>
    ...
    if ($detailElement.length) { // 해당 행의 detail이 이미 열려 있는 경우
        $detailElement.stop().slideUp(300, function() {
            $detailElement.remove();
        });
    } else { // 해당 행의 detail이 없는 경우
        if ($(".resource-detail").length > 0) { // 다른 곳이 열려 있는 경우
            $(".resource-detail").stop().slideUp(300, function() {
                $(this).remove();
                // 모든 기존 요소가 제거된 후 새로운 요소를 추가합니다
                $rowElement.after(html);
                $rowElement.next(".resource-detail").slideDown(300);
            });
        } else {
            // 기존 요소가 없으면 바로 새로운 요소를 추가합니다
            $rowElement.after(html);
            $rowElement.next(".resource-detail").slideDown(300);
        }
    }
...
function searchDataInit(resource_parent_no){
    ...
    console.log(resource_status_array.length);
...
function optimizerByPageInit() {
	...
	$('#explorer .content').jstree({
		...
		*/
		...
		$(".jstree .jstree-anchor").each(function(){
			if (!$(this).children(".btn-lazyload").length) {
				$(this).append(`<button class="btn-lazyload off">OFF</button>`);
			}
		});	
		$(".jstree .jstree-anchor .btn-lazyload").each(function(){
			$(this).click(function(){
...
function searchResourceList(){
	...
	for(let i = 0; i < reorderArray.length; i++){
		...
		html += `<li data-resource-org="${reorderArray[i].resource_org}" data-resource-seq="${i}" class="${reorderArray[i].class_name}">${resource_type} ${reorderArray[i].resource_name.substring(0, 18)}${dot} <span style="color:rgba(255,255,255,0.3);">(5회)</span><button onclick="drawResourceModal(${reorderArray[i].resource_no});"><ion-icon name="add-circle"></ion-icon></button></li>`;
	}
...
function resourcePrintByType(object){
	...
	switch(object.resource_type){
		case 1 : // 이미지
			result = {
				col1 : `<a href="view.do?image_path=${object.resource_new_type1}&image_name=" class="thumb preview-thumb image-popup"><img src="view.do?image_path=${object.resource_new_type1}&image_name=" /></a>`,
				col2 : `<a href="view.do?image_path=${object.resource_new_type2}&image_name=" class="thumb preview-thumb image-popup"><img src="view.do?image_path=${object.resource_new_type2}&image_name=" /></a>`, 
